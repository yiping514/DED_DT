...
from torch_optim import pytorch_optim
...
class GAMMA_MPC():
    def __init__(self,
        ...
        self.NN_pred_save_for_plot = torch.ones((1,P))*0
        ...
        self.PID_Kp = 0.025#0.1
        self.PID_Ki = 0.001 #0.01
        self.PID_Kd = 0.0001
    ...
    def MPC_run_one_step_pytorch(self):
        ...
        # optimization using pytorch minimize
        time1 = time.time()
        ...
        # solution_s_sigmoid = sigmoid(solution_s.x,torch.ones_like(solution_s.x),-torch.ones_like(solution_s.x))
        solution_s_sigmoid = solution_s.x
        solution = self.TiDE.inv_scaler_x(solution_s_sigmoid, dim_id = [3])
        
        # optimization using torch.optim
        # time1 = time.time()
        # solution_s = pytorch_optim(lambda u:self.obj(u,fix_cov_future_s,past_laser_power_s,fix_cov_past_s, mp_temp_past_part_s,mp_temp_ref_part_s,self.P,self.TiDE),torch.zeros((self.P,1),requires_grad=True), method="Adam")
        # time2 = time.time()
       
        # # scale solution to original scale
        # solution_s_sigmoid = sigmoid(solution_s,torch.ones_like(solution_s),-torch.ones_like(solution_s))
        # # solution_s_sigmoid = solution_s.x
        # solution = self.TiDE.inv_scaler_x(solution_s_sigmoid, dim_id = [3])
        ...
        # predict MP temp
        mp_hat_opt_s = self.TiDE.forward(solution_s_sigmoid, fix_cov_future_s, past_laser_power_s, fix_cov_past_s, mp_temp_past_part_s) # predicted MP temp
        # scale MP temp to original scale
        mp_hat_opt = self.TiDE.inv_scaler_y(mp_hat_opt_s,0)
        ...
        self.x_hat_current = mp_hat_opt[0]
        ...
        self.NN_pred_save_for_plot = torch.concatenate((self.NN_pred_save_for_plot,mp_hat_opt.reshape(1,-1)),dim=0)
