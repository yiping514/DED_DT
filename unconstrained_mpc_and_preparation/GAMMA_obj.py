...
from get_melt_pool_temp_width import get_meltpool_temp_width
...
class GAMMA_obj():
    ...
    def run_initial_steps(self,laser_power = None):
        ...
        for i in tqdm(range(sim_steps)):
            ...
            laser_on = 1
            self.melt_pool_temp_save, self.melt_pool_width_save = get_melt_pool(laser_on, self.GAMMA, self.global_counter, self.melt_pool_temp_save, self.melt_pool_width_save, self.solidus_temp)
        ...
        for i in range(0, len(mp_temp), 5):
            ...
            averaging = True
            if averaging == True:
                average_chunk = np.mean(chunk)
                
                if self.initial_average == None:
                    self.initial_average = np.mean(chunk)
                    
                averaged_sequence.append(0.5*(self.initial_average + average_chunk))
                self.initial_average = average_chunk
            else:
                average_chunk = np.mean(chunk)
                averaged_sequence.append(average_chunk)

        # Convert the list to a numpy array or keep it as a list based on your preference
        return np.array(averaged_sequence)
    ...
    def run_sim_interval(self,laser_power):
        ...
        if averaging == True:
            avg_mp_temp_output = 0.5*(avg_mp_temp + self.initial_average)
            self.initial_average = avg_mp_temp
            return avg_mp_temp_output   
        else:        
            return avg_mp_temp
...
def get_melt_pool(laser_on,GAMMA, timestep, melt_pool_temp_save, melt_pool_width_save, solidus_temp):
    if laser_on:
        melt_pool_width , melt_pool_temp = get_meltpool_temp_width(GAMMA, solidus_temp, timestep, None)
        ...
        return melt_pool_temp_save, melt_pool_width_save
    else:
