...
class heat_solve_mgr():
    def __init__(self, domain):
        ...
        self.laser_direction = domain.toolpath[self.current_step,0:3]*0
    ...
    def time_integration(self):
        ...
        if self.current_step == 0:
            self.prev_laser_location = None
        else:
            self.prev_laser_location = domain.toolpath[self.current_step-1,0:3]
            laser_moving_vec = self.laser_loc - self.prev_laser_location
            magnitude = np.sqrt(laser_moving_vec[0]**2 + laser_moving_vec[1]**2 + laser_moving_vec[2]**2)
            magnitude[magnitude == 0] = 1
              
            # unit_vectors = laser_moving_vec

            self.laser_direction = laser_moving_vec/magnitude
